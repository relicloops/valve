...
int vl_requirements_copy_table_(
  ...
  for (size_t i = 0; i < option_count; ++i) {
    ...
    const size_t requirement_count = option_requirement_count_(source);

    for (size_t j = 0; j < requirement_count; ++j) {
      const vl_option_t *target = option_mapped_(
          source->requires[j], source_globals, copied_globals, global_count,
      ...
      if (!target ||
          vl_requirement_add_(v, copied, target, verb, subverb) != 0)
        return -1;
      /* GCOVR_EXCL_BR_STOP */
    }
  }
