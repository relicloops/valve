...
void test_create_destroy(void) {
  ...
  EXPECT(v != NULL, "vl_create returns non-null");
  vl_destroy(v);
}

void test_destroy_null(void) {
  vl_destroy(NULL);
  EXPECT(1, "vl_destroy(NULL) is safe");
}

void test_version_get(void) {
  const char *s = vl_version_get();
  EXPECT(s != NULL && s[0] != '\0', "vl_version_get returns non-empty");
}
